...
SELECT
  ...
  i.dao_id,
  e.contract_id,
  (e.topics ->> 'token_id')::bigint AS token_id,
  e.topics ->> 'bidder' AS bidder,
  (e.args ->> 'amount')::numeric(78,0) AS amount,
  (e.args ->> 'extended')::boolean AS extended,
...
WITH config AS (
  SELECT DISTINCT ON (deployment_id, contract_id)
    deployment_id,
    contract_id,
    (e.args ->> 'duration')::bigint AS duration_seconds,
...
SELECT
  ...
  e.contract_id,
  (e.topics ->> 'token_id')::bigint AS token_id,
  (e.args ->> 'start_time')::bigint AS start_seconds,
  (e.args ->> 'end_time')::bigint AS end_seconds,
  c.duration_seconds,
  c.time_buffer_seconds,
  (e.args ->> 'reserve_price')::numeric(78,0) AS reserve_price,
  ...
  EXISTS (
    ...
    WHERE s.deployment_id = e.deployment_id
      ...
      AND s.token_id = (e.topics ->> 'token_id')::bigint
