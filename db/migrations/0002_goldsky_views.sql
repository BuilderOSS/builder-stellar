...
WITH c AS (SELECT event_id AS created_event_id, deployment_id, topics ->> 'proposal_id' AS proposal_id, topics ->> 'proposer' AS proposer,
  ...
  l AS (SELECT DISTINCT ON (deployment_id, proposal_id) deployment_id, proposal_id, state, eta_seconds, event_ledger AS updated_ledger, event_at AS updated_at
   FROM governance.proposal_lifecycle ORDER BY deployment_id, proposal_id, event_ledger DESC, lifecycle_event_id DESC)
 SELECT c.*, COALESCE(l.state,'pending') AS state, l.eta_seconds, l.updated_ledger, l.updated_at FROM c LEFT JOIN l USING (deployment_id, proposal_id);
...
SELECT event_id, deployment_id, topics ->> 'delegator' AS delegator, args ->> 'from_delegate' AS from_delegate,
  args ->> 'to_delegate' AS to_delegate, ledger_sequence AS event_ledger,
  extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds,
  to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash
...
CREATE OR REPLACE VIEW governance.governor_authority_history AS SELECT event_id, deployment_id, topics ->> 'authority' AS authority,
  ...
  extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds,
  to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role = 'governor' AND lower(event_name) IN ('governor_authority_changed','governorauthoritychanged');
CREATE OR REPLACE VIEW governance.governor_authorities AS SELECT DISTINCT ON (deployment_id, authority) deployment_id, authority, enabled, event_ledger, event_at, transaction_hash, 'event'::text AS source FROM governance.governor_authority_history WHERE enabled ORDER BY deployment_id, authority, event_ledger DESC;
CREATE OR REPLACE VIEW token.mint_authorities AS SELECT DISTINCT ON (deployment_id, authority) deployment_id, authority, enabled, event_ledger, event_at, transaction_hash, 'event'::text AS source FROM token.mint_authority_history WHERE enabled ORDER BY deployment_id, authority, event_ledger DESC;

 CREATE OR REPLACE VIEW auction.bids AS SELECT event_id, deployment_id, (topics ->> 'token_id')::bigint AS token_id, topics ->> 'bidder' AS bidder, (args ->> 'amount')::numeric(78,0) AS amount, (args ->> 'extended')::boolean AS extended, (args ->> 'new_end_time')::bigint AS new_end_seconds, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role = 'auction' AND lower(event_name) IN ('bid_placed','bidplaced');
 CREATE OR REPLACE VIEW auction.bid_refunds AS SELECT event_id, deployment_id, (topics ->> 'token_id')::bigint AS token_id, topics ->> 'bidder' AS bidder, (args ->> 'amount')::numeric(78,0) AS amount, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role = 'auction' AND lower(event_name) IN ('bid_refunded','bidrefunded');
 CREATE OR REPLACE VIEW auction.settlements AS SELECT event_id, deployment_id, (topics ->> 'token_id')::bigint AS token_id, args ->> 'winner' AS winner, (args ->> 'amount')::numeric(78,0) AS amount, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role = 'auction' AND lower(event_name) IN ('auction_settled','auctionsettled');
 ...
 SELECT e.event_id, e.deployment_id, (e.topics ->> 'token_id')::bigint AS token_id,
   ...
   EXISTS (SELECT 1 FROM auction.settlements s WHERE s.deployment_id=e.deployment_id AND s.token_id=(e.topics ->> 'token_id')::bigint AND s.event_ledger >= e.ledger_sequence) AS settled,
...
CREATE OR REPLACE VIEW treasury.calls AS SELECT event_id, deployment_id, topics ->> 'proposal_id' AS proposal_id, topics ->> 'governor' AS governor, topics ->> 'target' AS target, args ->> 'function' AS function, args -> 'args' AS args, args ->> 'executor' AS executor, args ->> 'action_index' AS action_index, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role='treasury' AND lower(event_name) IN ('execute','treasury_call_indexed','proposal_call_indexed');
...
CREATE OR REPLACE VIEW metadata.properties AS SELECT event_id, deployment_id, contract_id AS metadata_contract, (topic_0)::integer AS property_id, args ->> 'name' AS name, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role='metadata' AND lower(event_name) IN ('property_added','propertyadded');
CREATE OR REPLACE VIEW metadata.token_seeds AS SELECT event_id, deployment_id, contract_id AS metadata_contract, (topic_0)::bigint AS token_id, (args ->> 'num_properties')::integer AS num_properties, args -> 'selections' AS selections, ledger_sequence AS event_ledger, extract(epoch FROM to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000))::bigint AS event_timestamp_seconds, to_timestamp(NULLIF(ledger_closed_at,'')::numeric / 1000) AS event_at, transaction_hash FROM chain.decoded_events WHERE contract_role='metadata' AND lower(event_name) IN ('seed_generated','seedgenerated');
